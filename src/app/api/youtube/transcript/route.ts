...
async function fetchTranscript(
  ...
  try {
    ...
    const pythonScript = path.join(process.cwd(), 'fetch_transcript.py');
    const venvPython = path.join(process.cwd(), 'venv', 'bin', 'python3');
    
    console.log(`Executing Python script: ${venvPython} ${pythonScript} ${videoId}`);
    
    return new Promise((resolve, reject) => {
      const python = spawn(venvPython, [pythonScript, videoId]);
      ...
      python.on('close', (code: number) => {
        ...
        try {
          ...
          // Convert the single transcript text to segments
          // Split by sentences and create segments with estimated timing
          ...
          console.log(`Transcript processed: ${segments.length} segments`);
